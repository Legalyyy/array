...
const OWNER_USER_ID = "404315406111604747";
...
async function getRandomBibleVerse() {
  ...
  if (!booksResponse.ok) {
    ...
    throw new Error(`Bible API error (${booksResponse.status}): ${errorText.substring(0, 200)}`);
  }
  ...
  if (!chaptersResponse.ok) {
    throw new Error(`Bible API chapters error: ${chaptersResponse.status}`);
  }
  ...
  if (!versesResponse.ok) {
    throw new Error(`Bible API verses error: ${versesResponse.status}`);
  }
  ...
  if (verses.length === 0) {
    ...
    if (!chapterResponse.ok) {
      throw new Error(`Bible API chapter error: ${chapterResponse.status}`);
    }
  ...
  if (!verseResponse.ok) {
    throw new Error(`Bible API verse text error: ${verseResponse.status}`);
  }
...
async function handleTestApisCommand(interaction: ChatInputCommandInteraction) {
  try {
    ...
    try {
      ...
      bibleStatus = "❌ Failed";
      bibleError = error.message || String(error);
    }
