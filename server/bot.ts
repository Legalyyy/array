...
// Create Discord client with all necessary intents for comprehensive logging
export const client = new Client({
  intents: [
    ...
    GatewayIntentBits.GuildIntegrations, // For integration events
    GatewayIntentBits.GuildInvites, // For invite events
...
const commands = [
  ...
  new SlashCommandBuilder()
    .setName("setlogs")
    .setDescription("admin - set the channel for comprehensive server logs")
    .addChannelOption(option =>
      option
        .setName("channel")
        .setDescription("Channel to send all server logs to")
        .setRequired(true)
    )
    .setDefaultMemberPermissions(PermissionsBitField.Flags.Administrator),
  ...
  new SlashCommandBuilder()
    .setName("rigor")
    .setDescription("admin - manage scheduled messages")
    ...
    .addSubcommand(subcommand =>
      subcommand
        ...
        .addIntegerOption(option =>
          option
            .setName("interval")
            .setDescription("How often to send (in minutes)")
            .setRequired(true)
            .setMinValue(1)
        )
    )
...
client.once(Events.ClientReady, async (c) => {
  ...
  console.log("✅ Setting up logging event listeners...");
  
  // Guild events
  client.on('guildUpdate', (oldGuild, newGuild) => logging.logGuildUpdate(oldGuild, newGuild, storage));
  client.on('guildBanAdd', (ban) => logging.logGuildBanAdd(ban, storage));
  client.on('guildBanRemove', (ban) => logging.logGuildBanRemove(ban, storage));
  
  // Channel events
  client.on('channelCreate', (channel) => logging.logChannelCreate(channel, storage));
  client.on('channelDelete', (channel) => logging.logChannelDelete(channel, storage));
  client.on('channelUpdate', (oldChannel, newChannel) => logging.logChannelUpdate(oldChannel, newChannel, storage));
  
  // Role events
  client.on('roleCreate', (role) => logging.logRoleCreate(role, storage));
  client.on('roleDelete', (role) => logging.logRoleDelete(role, storage));
  client.on('roleUpdate', (oldRole, newRole) => logging.logRoleUpdate(oldRole, newRole, storage));
  
  // Member events
  client.on('guildMemberAdd', (member) => logging.logMemberJoin(member, storage));
  client.on('guildMemberRemove', (member) => logging.logMemberLeave(member, storage));
  client.on('guildMemberUpdate', (oldMember, newMember) => logging.logMemberUpdate(oldMember, newMember, storage));
  
  // Message events
  client.on('messageDelete', (message) => logging.logMessageDelete(message, storage));
  client.on('messageUpdate', (oldMessage, newMessage) => logging.logMessageEdit(oldMessage, newMessage, storage));
  client.on('messageDeleteBulk', (messages) => logging.logMessageBulkDelete(messages, storage));
  
  // Voice events
  client.on('voiceStateUpdate', (oldState, newState) => logging.logVoiceStateUpdate(oldState, newState, storage));
  
  // Emoji events
  client.on('emojiCreate', (emoji) => logging.logEmojiCreate(emoji, storage));
  client.on('emojiDelete', (emoji) => logging.logEmojiDelete(emoji, storage));
  client.on('emojiUpdate', (oldEmoji, newEmoji) => logging.logEmojiUpdate(oldEmoji, newEmoji, storage));
  
  // Sticker events
  client.on('stickerCreate', (sticker) => logging.logStickerCreate(sticker, storage));
  client.on('stickerDelete', (sticker) => logging.logStickerDelete(sticker, storage));
  client.on('stickerUpdate', (oldSticker, newSticker) => logging.logStickerUpdate(oldSticker, newSticker, storage));
  
  // Invite events
  client.on('inviteCreate', (invite) => logging.logInviteCreate(invite, storage));
  client.on('inviteDelete', (invite) => logging.logInviteDelete(invite, storage));
  
  // Thread events
  client.on('threadCreate', (thread) => logging.logThreadCreate(thread, storage));
  client.on('threadDelete', (thread) => logging.logThreadDelete(thread, storage));
  client.on('threadUpdate', (oldThread, newThread) => logging.logThreadUpdate(oldThread, newThread, storage));
  
  // Scheduled event events
  client.on('guildScheduledEventCreate', (event) => logging.logScheduledEventCreate(event, storage));
  client.on('guildScheduledEventDelete', (event) => logging.logScheduledEventDelete(event, storage));
  client.on('guildScheduledEventUpdate', (oldEvent, newEvent) => logging.logScheduledEventUpdate(oldEvent, newEvent, storage));
  
  // Auto mod events
  client.on('autoModerationRuleCreate', (rule) => logging.logAutoModRuleCreate(rule, storage));
  client.on('autoModerationRuleDelete', (rule) => logging.logAutoModRuleDelete(rule, storage));
  client.on('autoModerationRuleUpdate', (oldRule, newRule) => logging.logAutoModRuleUpdate(oldRule, newRule, storage));
  client.on('autoModerationActionExecution', (execution) => logging.logAutoModActionExecution(execution, storage));
  
  // Stage instance events
  client.on('stageInstanceCreate', (instance) => logging.logStageInstanceCreate(instance, storage));
  client.on('stageInstanceDelete', (instance) => logging.logStageInstanceDelete(instance, storage));
  client.on('stageInstanceUpdate', (oldInstance, newInstance) => logging.logStageInstanceUpdate(oldInstance, newInstance, storage));
  
  console.log("✅ Logging event listeners set up!");
});
  ...
  try {
    switch (commandName) {
      ...
      case "setlogs":
        ...
        break;
      case "rigor":
        await handleRigorCommand(interaction);
        break;
    }
  } catch (error) {
...
async function handleAdminCommand(message: Message, userMessage: string): Promise<boolean> {
  ...
  // SEND DM TO USER - Flexible command for sending DMs
  if (/(?:send|envia|enviar|manda|mandar).*(?:dm|direct message|mensaje directo|private message)/i.test(lowerMessage)) {
    const targetResult = getTargetUser(message);
    const mentionedUser = targetResult?.user;
    
    if (!mentionedUser) {
      await message.reply(`❌ You need to mention a user to send them a DM.`);
      return true;
    }

    // Extract message content after "saying" or similar keywords
    const messageMatch = userMessage.match(/(?:saying|diciendo|que diga|that says?)\s+(.+)/i);
    
    if (!messageMatch) {
      await message.reply(`❌ Please specify what message to send using "saying [message]"`);
      return true;
    }

    const dmContent = messageMatch[1].trim();

    try {
      await mentionedUser.send(dmContent);
      await message.reply(`✅ DM sent to ${mentionedUser.tag}`);
      console.log(`✅ ${message.author.username} sent DM to ${mentionedUser.tag}: "${dmContent}"`);
      return true;
    } catch (error) {
      console.error("Error sending DM:", error);
      await message.reply(`❌ Failed to send DM. They may have DMs disabled.`);
      return true;
    }
  }

  // SEND MESSAGE TO CHANNEL - Flexible command for sending messages to specific channels
  if (/(?:send|envia|enviar|manda|mandar).*(?:message|mensaje).*(?:to|al?|en)/i.test(lowerMessage) &&
      !/(dm|direct)/i.test(lowerMessage)) {
    
    const channelMention = message.mentions.channels.first();
    
    if (!channelMention) {
      await message.reply(`❌ You need to mention a channel to send a message to it.`);
      return true;
    }
    ...
    try {
      const channel = await message.guild?.channels.fetch(channelMention.id);
      if (channel && channel.isTextBased()) {
        await channel.send(messageContent);
        await message.reply(`✅ Message sent to ${channelMention}`);
        console.log(`✅ ${message.author.username} sent message to ${channelMention.name}: "${messageContent}"`);
        return true;
      } else {
        await message.reply(`❌ That channel doesn't support text messages.`);
        return true;
      }
    } catch (error) {
      console.error("Error sending message to channel:", error);
      await message.reply(`❌ Failed to send message to channel.`);
      return true;
    }
  }
...
async function handleSetLogsCommand(interaction: ChatInputCommandInteraction) {
  try {
    ...
    await interaction.reply({
      content: `✅ Logs channel set to ${channel}. All server events will be logged there.`,
      ephemeral: true
    });

    console.log(`✅ ${interaction.user.username} set logs channel to ${channel.id} in ${interaction.guild.name}`);
  } catch (error) {
    console.error("Error in setlogs command:", error);
    await interaction.reply({ content: "❌ Failed to set logs channel.", ephemeral: true });
  }
}

// Handle /rigor command
async function handleRigorCommand(interaction: ChatInputCommandInteraction) {
  try {
    if (!interaction.guild) {
      await interaction.reply({ content: "❌ This command can only be used in a server.", ephemeral: true });
      return;
    ...
    if (subcommand === "list") {
      const scheduledMessages = await storage.getScheduledMessages(interaction.guild.id);
      
      if (scheduledMessages.length === 0) {
        await interaction.reply({
          content: "📋 No scheduled messages found. Use `/rigor add` to create one.",
          ephemeral: true
        });
        return;
      }
      ...
      const embed = new EmbedBuilder()
        .setColor(0x2B5BBA)
        .setTitle("📋 Scheduled Messages (Rigor)")
        .setDescription(`Total: ${scheduledMessages.length} scheduled messages`)
        .setTimestamp();

      scheduledMessages.forEach((msg, index) => {
        const channel = interaction.guild?.channels.cache.get(msg.channelId);
        const nextSend = msg.lastSentAt 
          ? new Date(msg.lastSentAt.getTime() + msg.intervalMinutes * 60000)
          : new Date();
        
        embed.addFields({
          name: `ID: ${msg.id} - ${msg.isActive ? "✅ Active" : "❌ Inactive"}`,
          value: `**Channel:** ${channel || `<#${msg.channelId}>`}\n**Message:** ${msg.message.substring(0, 100)}${msg.message.length > 100 ? '...' : ''}\n**Interval:** Every ${msg.intervalMinutes} minutes\n**Next Send:** <t:${Math.floor(nextSend.getTime() / 1000)}:R>\n**Created by:** ${msg.createdByUsername}`,
          inline: false
        });
      });
      ...
      const scheduledMsg = await storage.addScheduledMessage({
        guildId: interaction.guild.id,
      ...
      const embed = new EmbedBuilder()
        ...
        .addFields(
          { name: "ID", value: scheduledMsg.id.toString(), inline: true },
      ...
      await interaction.reply({ embeds: [embed], ephemeral: true });
      console.log(`✅ ${interaction.user.username} created scheduled message ID ${scheduledMsg.id}`);
    }
    else if (subcommand === "delete") {
      const id = interaction.options.getInteger("id", true);

      const scheduledMsg = await storage.getScheduledMessageById(id);
      
      if (!scheduledMsg) {
        await interaction.reply({ content: `❌ Scheduled message with ID ${id} not found.`, ephemeral: true });
        return;
      }

      if (scheduledMsg.guildId !== interaction.guild.id) {
        await interaction.reply({ content: `❌ That scheduled message doesn't belong to this server.`, ephemeral: true });
        return;
      }

      await storage.deleteScheduledMessage(id);
      await interaction.reply({ content: `✅ Deleted scheduled message ID ${id}`, ephemeral: true });
      console.log(`✅ ${interaction.user.username} deleted scheduled message ID ${id}`);
    }
  } catch (error) {
    console.error("Error in rigor command:", error);
    await interaction.reply({ content: "❌ Failed to execute command.", ephemeral: true }).catch(() => {});
  }
}

// Scheduled messages processor (runs every minute)
setInterval(async () => {
  try {
    ...
    for (const msg of activeMessages) {
      const lastSent = msg.lastSentAt ? new Date(msg.lastSentAt) : new Date(0);
      const nextSend = new Date(lastSent.getTime() + msg.intervalMinutes * 60000);
      ...
      if (now >= nextSend) {
        try {
          ...
          if (channel && channel.isTextBased()) {
            ...
            await storage.updateScheduledMessageLastSent(msg.id);
            console.log(`✅ Sent scheduled message ID ${msg.id} to channel ${msg.channelId}`);
          }
        } catch (error) {
