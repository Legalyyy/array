...
export interface IStorage {
  ...
  // Welcomed Members
  addWelcomedMember(member: InsertWelcomedMember): Promise<WelcomedMember>;
  hasBeenWelcomed(guildId: string, userId: string): Promise<boolean>;
  deleteWelcomedMember(guildId: string, userId: string): Promise<void>;
  
  // Scheduled Messages (Rigor)
  addScheduledMessage(message: InsertScheduledMessage): Promise<ScheduledMessage>;
  getScheduledMessages(guildId: string): Promise<ScheduledMessage[]>;
  getActiveScheduledMessages(): Promise<ScheduledMessage[]>;
  getScheduledMessageById(id: number): Promise<ScheduledMessage | undefined>;
  deleteScheduledMessage(id: number): Promise<void>;
  updateScheduledMessageLastSent(id: number): Promise<void>;
  toggleScheduledMessageActive(id: number, isActive: boolean): Promise<ScheduledMessage | undefined>;
}
...
export class DatabaseStorage implements IStorage {
  ...
  // Welcomed Members
  async addWelcomedMember(member: InsertWelcomedMember): Promise<WelcomedMember> {
    const [result] = await db
      .insert(welcomedMembers)
      .values(member)
      .returning();
    return result;
  }
  ...
  async hasBeenWelcomed(guildId: string, userId: string): Promise<boolean> {
    const [result] = await db
      .select()
      .from(welcomedMembers)
      .where(and(
        eq(welcomedMembers.guildId, guildId),
        eq(welcomedMembers.userId, userId)
      ));
    return !!result;
  }

  async deleteWelcomedMember(guildId: string, userId: string): Promise<void> {
    await db
      .delete(welcomedMembers)
      .where(and(
        eq(welcomedMembers.guildId, guildId),
        eq(welcomedMembers.userId, userId)
      ));
  ...
  // Scheduled Messages (Rigor)
  async addScheduledMessage(message: InsertScheduledMessage): Promise<ScheduledMessage> {
    const [result] = await db
  ...
  async getScheduledMessageById(id: number): Promise<ScheduledMessage | undefined> {
    const [result] = await db
      .select()
      .from(scheduledMessages)
  ...
  async updateScheduledMessageLastSent(id: number): Promise<void> {
    await db
  ...
  async toggleScheduledMessageActive(id: number, isActive: boolean): Promise<ScheduledMessage | undefined> {
    const [result] = await db
      .update(scheduledMessages)
